...
?>
  <section class="home d-flex align-items-center">
    <div class="home-content text-white   ms-5 ">
      <h3 class="fa-4x">Buy NOW </h3>
      <button class="btn btn-light py-2 me-1" onclick="window.location.href='renting-page.php'">Rent NOW</button>
        ...
        <div class="col-md-12  ">
          ...
          <div class="row">
            <?php
              include_once '../DBCODE/Database.php';  
              $db = new Database();	
              if($q<=1){$rs = $db->GetData(" select * from house where (status='Sell') limit 9"); }
              elseif ($q==2) {
                $rs = $db->GetData(" select * from house where (status='Sell') limit 9,9");   }
            elseif($q>2){
              $j=$q-1;
              $k=9* $j;
              $rs = $db->GetData(" select * from house where (status='Sell') limit $k,9");   
            }
...
?>
            ...
            <div class="p-3 text-center">
              ...
              <div class="text-center">
                <i class="fas fa-map-marker-alt fa-lg m-2"> <span>
  ...
  <div class="d-flex justify-content-center">
    <nav aria-label="Page navigation example">
      <ul class="pagination">
        ...
        <?php
                     $rs1 = $db->GetData(" select count(*) from house where (status='Sell')"); 
                    ...
                    for ($x = 1; $x <= ($y) ;  $x++) {echo "<li class='page-item'><a class='page-link' href='buy.php?q=$x #home' ><button class='btn btn-sm'  type='submit' id='page' name='pg' onclick='pagecount(".$x.")'>".$x."</button></a></li>"; } }?>
